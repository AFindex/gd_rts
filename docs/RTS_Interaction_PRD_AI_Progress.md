...
- 改造：[scripts/core/game_manager.gd](D:/Godot/projs/gd_rts/scripts/core/game_manager.gd)，新增异质单位子组切换（`Ctrl+Tab` 循环类型），并将单位指令下发限制到激活子组。
...
10. 混合单位类型（如工人+士兵）按 `Ctrl+Tab`：应循环子组，后续移动/攻击/停止仅作用于当前激活子组。
...
- 已有编组、子组、同类双击、编组数字地标与多选分页基础版；仍缺更完整子组头像交互（例如可点击子组头像切换）。
...
| 4.7 选中与群组     | 部分实现 | 已支持单选/框选/混选基础 + 控制编组（设置/追加/选中/双击聚焦）+ 编组数字地标 + `Ctrl+Tab` 子组循环与子组指令控制 + 双击同类/`Ctrl+双击` 全图同类 + 子组矩阵高亮 + 多选分页（PgUp/PgDn） | 缺更完整子组头像交互 |
...
| A-008 | 控制编组与子组切换              | P1  | 进行中（收尾） | 已支持 Ctrl设组/Shift追加/数字选组与双击聚焦，已支持编组数字地标，已支持 Ctrl+Tab 子组轮换与子组指令控制，已支持双击同类/`Ctrl+双击` 全图同类、子组矩阵高亮、PgUp/PgDn 多选分页与点击矩阵切换子组；待补更完整子组头像交互 |
...
1. 先补 `A-008` 最后一项：可点击子组头像/子组条交互（目前已具备分页与子组联动基础）。
2. 再补 `A-007` 收尾：接入正式图标资源（矿/剑/盾/中继）与分层音效（确认/错误/警示）。
3. 然后补齐 `A-005` 剩余项（右键按住预览、维修/治疗分支、非法反馈音效）。
