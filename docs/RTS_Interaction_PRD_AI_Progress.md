...
- 右键智能判定链不完整（无跟随/维修/集结点/交付优先级等）。
- 单位命令队列已支持基础排队，但路径点可视化、队列编辑、上限控制尚未实现。
...
| 4.3 智能右键 | 部分实现 | 资源/敌人/地面三类判定 | 缺严格优先级链、跟随/维修/交付/集结点、按住右键预览与冲突解算 |
| 4.4 建造放置 | 部分实现 | 虚影预览、合法红绿、确认放置、ESC/RMB取消 | 缺旋转、间距数值、范围圈、工人移动到位后开建、连续建造 |
| 4.5 集结点系统 | 未实现 | - | 缺全部集结点类型、多跳中继、可视化连线 |
| 4.6 命令队列系统 | 部分实现 | 已支持单位命令排队（Shift+RMB/目标技能点击） | 缺路径点编号可视化、混合编辑、上限与插入删除 |
...
| A-004 | `Shift` 队列下达与基础可视化 | P0 | 进行中 | 队列下达已支持，可视化路径点未做 |
| A-005 | 智能右键优先级链 | P0 | 待开始 | 对应 PRD 4.3.1 |
| A-006 | 建造旋转与连续建造 | P1 | 待开始 | 对应 PRD 4.4 |
| A-007 | 集结点系统（地面/资源/攻击） | P1 | 待开始 | 对应 PRD 4.5 |
...
1. 先完成 `A-004`：补齐队列路径点可视化、队列上限与编辑能力。
2. 再做 `A-005`：实现右键优先级链与冲突解算，形成完整 Smart Command。
3. 然后推进 `A-006 + A-007 + A-008`（建造深度、集结点、编组与子组）。
