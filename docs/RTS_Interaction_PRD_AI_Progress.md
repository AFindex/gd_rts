...
- 已有编组与子组基础版；仍缺双击同类全选、Ctrl双击全图同类及更完整子组UI。
...
| 4.7 选中与群组     | 部分实现 | 已支持单选/框选/混选基础 + 控制编组（设置/追加/选中/双击聚焦）+ `Ctrl+Tab` 子组循环与子组指令控制 | 缺双击同类、Ctrl双击全图同类与更完整子组头像UI |
...
| A-008 | 控制编组与子组切换              | P1  | 进行中（基础版） | 已支持 Ctrl设组/Shift追加/数字选组与双击聚焦，已支持 Ctrl+Tab 子组轮换与子组指令控制；待补同类双击选择与子组头像UI |
...
1. 先补 `A-008` 收尾：同类双击/全图同类选择与子组头像UI联动。
