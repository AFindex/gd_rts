...
- 集结点已支持多跳中继（最多3跳）、基础可视化、受击闪烁警示、目标失效地面回退；仍缺旗帜图标细分与更完整告警音效。
...
| 4.5 集结点系统     | 部分实现 | 已支持生产建筑RMB设置集结（地面/资源/攻击/跟随/中继），`Shift+RMB` 追加最多3跳，并在出生后按跳点顺序执行；含连线+旗帜可视化、受击闪烁、目标失效回退 | 缺旗帜图标细分与更完整告警/音效                      |
...
| A-007 | 集结点系统（地面/资源/攻击）        | P1  | 进行中（增强版） | 已支持3跳中继、`Shift+RMB` 追加、连线旗帜可视化、受击闪烁、目标失效回退与出生按链执行，待补图标细分与告警音效 |
...
1. 先补 `A-007` 收尾：旗帜图标细分（矿/剑/盾/中继）和对应提示音。
2. 再推进 `A-008`（编组与子组切换）。
