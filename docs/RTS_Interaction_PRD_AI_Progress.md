...
- 已有编组、子组与同类双击基础版；仍缺更完整子组头像/分页交互与更细粒度编组可视化。
...
| 4.7 选中与群组     | 部分实现 | 已支持单选/框选/混选基础 + 控制编组（设置/追加/选中/双击聚焦）+ `Ctrl+Tab` 子组循环与子组指令控制 + 双击同类/`Ctrl+双击` 全图同类 + 子组矩阵高亮联动 | 缺更完整子组头像/分页交互与编组可视化 |
...
| A-008 | 控制编组与子组切换              | P1  | 进行中（增强版） | 已支持 Ctrl设组/Shift追加/数字选组与双击聚焦，已支持 Ctrl+Tab 子组轮换与子组指令控制，已支持双击同类/`Ctrl+双击` 全图同类与子组矩阵高亮；待补更完整子组头像/分页交互与编组可视化 |
...
1. 先补 `A-008` 收尾：编组地面数字标识与子组分页/头像增强。
